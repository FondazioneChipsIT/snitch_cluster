...
/* Matrix multiplication dst = trans(A)*B, only with square matrices */
#include "snrt.h"
#include "data.h"
...
int main() {
    ...
    if (snrt_is_dm_core()) {
        ...
        mat_b = trans_a + elems * elems;
        dst = mat_b + elems * elems;

        if (!mat_a || !trans_a || !mat_b || !dst) {
    ...
    if(snrt_is_compute_core()){
        ...
        matrix_trans_opt(chunk_per_core, offset, mat_a, trans_a); 
        // Multiplication
        matrix_mul_opt(chunk_per_core,offset,&start_cycle[core_idx], &end_cycle[core_idx], trans_a, mat_b, dst);
    ...
    if (core_idx == 0) {
        ...
        printf("Matrix multiplication transpose A %dx%d performance\n",elems,elems);
