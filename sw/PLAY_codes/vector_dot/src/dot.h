...
// Modified by Luca Colombo 2025 Chips-IT
...
static inline void dot(uint32_t n, double *x, double *y, double *result) {
    ...
    flop_cycle[core_idx] = (double) 2 *frac_core/(double) total_cycles[core_idx];
    ...
    if(snrt_cluster_core_idx()==0){
        ...
        for(uint32_t cid = 0; cid < snrt_cluster_compute_core_num(); cid ++){
            mean_cycles += total_cycles[cid];
            mean_flop_cycle += flop_cycle[cid];
        }
        mean_cycles /= snrt_cluster_compute_core_num();
        mean_flop_cycle /= snrt_cluster_compute_core_num();
