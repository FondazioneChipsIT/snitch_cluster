...
void vect_offset_opt(uint32_t chunk_per_core, uint32_t offset, uint64_t *start_cycle,uint64_t *end_cycle, 
    double *src, double *dst, double *offset_val){
    ...
    *start_cycle = snrt_mcycle();
    ...
    asm volatile(
        "fld ft3, 0(%[offset_val])\n"
        :                   // Outputs
        : [offset_val] "r"(offset_val)    // Inputs 
        : "ft3");           // Clobber list

    // Enable the SSRs
    snrt_ssr_enable();
    
    // Assembly code to add ft3 to ft0 and store in ft1
    asm volatile(
        "frep.o %[n_frep], 1, 0, 0 \n"
        "fadd.d ft1, ft0, ft3\n"
        :
