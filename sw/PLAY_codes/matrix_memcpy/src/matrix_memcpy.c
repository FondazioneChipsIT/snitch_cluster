...
/* Copies matrix src in matrix dst */

// Snitch runtime library
#include "snrt.h"
#include "data.h"
#include "matrix_memcpy_opt.h"
...
int main(){
    ...
    if(snrt_is_dm_core()){
        ...
        dst = src + rows*col; // The matrix is put in L1 by rows, so we need to allocate
        // rows*col elements
    ...
    if(snrt_is_compute_core()){
        ...
        matrix_memcpy_opt(chunk_per_core, offset, &start_cycle[core_idx], &end_cycle[core_idx],
                          src, dst);
    ...
    if(core_idx==0){
        ...
        printf("Matrix memcpy %dx%d performance\n",rows,col);
        ...
        /* for(uint32_t i=0; i<rows; i++){
            for(uint32_t j=0; j<col;j++){
                ...
                printf("mat_a(%d,%d): %f",i,j,mat_a[i * cols + j]);
