...
/* Copy src in dst 
   (We really do not copy as you cannot explicity store with ssr, we store in
   the dst src+0)  */

// Snitch runtime library
#include "snrt.h"
#include "data.h"
#include "vect_memcpy_opt.h"
...
int main(){
    ...
    if(snrt_is_compute_core()){
        ...
        // Call the kernel
        vect_memcpy_opt(chunk_per_core, offset, &start_cycle[core_idx], &end_cycle[core_idx],
                    src, dst);
    ...
    if(core_idx==0){
        ...
        printf("Vector memcopy %d performance\n",LEN);
