...
static inline void swap_rows_simple(double *mat, uint32_t n, uint32_t r1, uint32_t r2);

void lu_decomp_opt(uint32_t core_idx ,uint32_t ncores, uint64_t *start_cycle, uint64_t *end_cycle, 
                        double *mat, int *perm, 
                        double *row_k, double *row_b, double *vec_write_back) {
    ...
    *start_cycle = snrt_mcycle();
    ...
    for (uint32_t k = 0; k < elems; k++) {
        ...
        double pivot = mat[k * elems + k];
        double p_inv = 1.0 / pivot;
        ...
        if(snrt_is_compute_core()){
            ...
            for (int m = offset; m < end; m++) {
                ...
                snrt_ssr_loop_1d(SNRT_SSR_DM0, row_chunk, sizeof(double));
                snrt_ssr_loop_1d(SNRT_SSR_DM1, row_chunk, sizeof(double));
                snrt_ssr_loop_1d(SNRT_SSR_DM2, row_chunk, sizeof(double));
