...
void vect_set_all_opt(uint32_t chunk_per_core, uint32_t offset, uint64_t *start_cycle,uint64_t *end_cycle, 
    double *vec, double *oval){
    ...
    *start_cycle = snrt_mcycle();
    ...
    // Write to ft0
    snrt_ssr_write(SNRT_SSR_DM0, SNRT_SSR_1D, vec + offset); //ft0->vec
